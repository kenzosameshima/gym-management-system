...
import { canManagePlans } from "../auth/roleAccess";
...
import { formatCurrency, formatFinancialStatus, getErrorMessage, STATUS_OPTIONS } from "./pageUtils";

const EMPTY_FORM: PlanPayload = { name: "", price: "", duration_days: 30, status: "ACTIVE" };

export function PlansPage(): JSX.Element {
  const { user } = useAuth();
  const canWrite = canManagePlans(user?.role);
  ...
  const columns: Column<Plan>[] = [
    ...
    { key: "status", header: "Status", render: (plan) => formatFinancialStatus(plan.status) }
  ];
  if (canWrite) {
    columns.push({
      key: "actions",
